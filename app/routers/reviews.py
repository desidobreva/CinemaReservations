...
def list_reviews(movie_id: int, db: Session = Depends(get_db)) -> list[ReviewOut]:
    if not db.get(Movie, movie_id):
        raise HTTPException(status_code=404, detail="Movie not found")

    rows = db.query(Review).filter(Review.movie_id == movie_id).order_by(Review.id.desc()).all()
